       ...
       ![Tools isNullOrEmptyString:token] &&  uid != 0 && self.currentPublication != nil){
        ...
        if(observation && placeName && comments && date ){
            ...
            if(_nid > 0 ){
                ...
                query = [NSString stringWithFormat:@"UPDATE $T SET  _placeName = '%@' , _title = '%@' , _speciesCount = '%li' ,_modifiedTime = '%f' ,_date = '%f' ,_isSynced = '0' WHERE _nid = '%li' ", _placeName,_title,(long)_count,_modified,_date,(long)_nid];
            }else{
                //---- Update by _uuid : tsy mbola synced sady tsy nahazo _nid avy any @ server
                query = [NSString stringWithFormat:@"UPDATE $T SET  _placeName = '%@' , _title = '%@' , _speciesCount = '%li' ,_modifiedTime = '%f' ,_date = '%f' ,_isSynced = '0' WHERE _uuid = '%@' ", _placeName,_title,(long)_count,_modified,_date,_uuid];
